 ...
 */
window.initS2ToggleAll = function() {};
window.initS2Order = function() {};
window.initS2Loading = function() {};
window.initS2Change = function() {};
window.initS2Unselect = function() {};
(function(factory) {
    'use strict';
    ...
    initS2ToggleAll = function(id) {
        ...
        $el.on('select2:open.krajees2', function() {
            if (
                $tog.parent().attr('id') === 'parent-' + togId ||
                !$el.attr('multiple')
            ) {
                return;
            }
            ...
            $('#select2-' + id + '-results')
                ...
                .each(function(k, v) {
                    var elemData = $(v).data('data'),
                        optElem =
                            typeof elemData.element !== 'undefined'
                                ? $(elemData.element)
                                : [];
    ...
    initS2Change = function($el) {
        $el = $el || $(this);
        var $drop = $('.select2-container--open'),
            cssClasses,
            i,
            $src = $el.parents("[class*='has-']");
        if ($src.length) {
    ...
    initS2Loading = function(id, optVar) {
         ...
         */
        var opts = window[optVar] || {},
            themeCss = opts.themeCss,
            sizeCss = opts.sizeCss,
            doOrder = opts.doOrder,
            doReset = opts.doReset,
            doToggle = opts.doToggle,
            $el = $('#' + id),
            $container = $(themeCss),
        ...
        if ($group.length) {
            $group
                .removeClass('kv-input-group-hide')
                .removeClass('.group-' + id);
        }
        if ($loading.length) {
            $loading.remove();
        }
